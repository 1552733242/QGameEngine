...
void Sandbox2D::OnUpdate(QGame::Timestep ts)
{
...
#if 1
	{
		...
		QGame::Renderer2D::DrawQuare({ 0.5f,-0.5f }, { 0.5f,0.75f }, m_SquareColor);
		QGame::Renderer2D::DrawQuare({ 0.0f,0.0f,-0.1f }, { 20.0f,20.0f }, m_CheckborardTexture, 10.f);
		QGame::Renderer2D::DrawQuare({ -2.0f,0.0f }, { 1.0f,1.0f },rotation, m_CheckborardTexture, 20.f);
		...
		for (float y = -5.0f; y < 5.0f; y += 0.5f) {
			for (float x = -5.0f; x < 5.0f; x += 0.5f) {
				...
				QGame::Renderer2D::DrawQuare({ x,y }, { 0.45f,0.45f }, color);
			}
		}
